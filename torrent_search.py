...
"""Torrent search across public indexers and the user's own Torznab feeds.

Ported from blindDL, which shares an author with this program. The indexer
list, the normalized result row and the parallel-with-a-deadline search all
come from there; what is dropped is blindDL's hand-off to an external
BitTorrent client, because SerrebiTorrent is that client -- a chosen result
goes straight into the connected session.

Several projects pointed the way originally. sayem314/torrent-indexer
supplied the list of indexers worth asking and the idea of one normalized
result whatever the site; WhitlockXD/moviepilot-custom singled out the public
Pirate Bay and BitSearch endpoints; Wizzel-F50/Limetorrents-addon the
LimeTorrents search path; SimoneFelici/bt1337xearch the category and seeder
ordering a torrent list needs; focarica/AutoTorrent the plain "search, pick,
fetch" shape; and araidz/Trawl the two that matter most, Knaben and
SolidTorrents.
...
Nothing here touches the session. It searches, and it hands back either a
magnet link or the bytes of a .torrent file.

The `prefs` argument throughout is SerrebiTorrent's preferences dict, which
carries `torznab_feeds` and `disabled_torrent_sources`.
"""
...
def score_match(query, title):
    """How well one result answers the query, 0-100.

    Release names are not titles -- "The Office S03E12 1080p WEB-DL" has to
    score well against "the office" -- so a query whose every word appears in
    the name is treated as a strong match, with the plain similarity kept in
    the number so that better-matching names still sort first.
    """
    score = _token_sort_ratio(query, title)
    query_tokens = set(_tokens(query))
    if query_tokens and query_tokens <= set(_tokens(title)):
        score = max(score, 70.0 + 0.3 * score)
...
# SerrebiTorrent never asks for a tracker password.
...
def all_sources(prefs=None):
    """Every indexer available: the built-in ones plus the user's feeds."""
    return list(ALL_SOURCES) + [feed["name"] for feed in feeds(prefs)]


def sources_by_label(prefs=None):
    """Every indexer, ordered the way a list should read."""
    return sorted(all_sources(prefs), key=str.lower)
...
def _item(source, infohash, title, **extra):
    """One normalized result row, the same shape whatever indexer it came from."""
    ...
    item = {
        ...
        "size_bytes": 0,
        "file_size": "",
        # "age" is what the list shows; "posted" is the unix time behind it,
        # kept so the Newest sort has a number to work with. Indexers that
        # date their rows only in words leave it at 0.
        "age": "",
        "posted": 0,
        "url": "",
    }
...
def fetch_torrent_bytes(item, timeout=HTTP_TIMEOUT_S):
    """Download one result's .torrent file and return its bytes.

    Private trackers publish an authenticated .torrent rather than a magnet,
    and the URL only works for the account that was given it. Fetching the
    file is what carries the tracker's passkey through to the session; handing
    it the URL would not.
    """
...
def resolve(item, timeout=HTTP_TIMEOUT_S):
    """What to hand the session for one result: ("magnet", str) or ("file", bytes).

    A magnet needs nothing fetched, so it wins wherever there is one. The
    rest are private trackers, whose .torrent has to be downloaded with the
    account's own credentials before the session can be given anything.
    """
    magnet = magnet_for(item)
    if magnet:
        return "magnet", magnet
    if item.get("download_url"):
        return "file", fetch_torrent_bytes(item, timeout=timeout)
...
def search_nyaa(query, timeout=HTTP_TIMEOUT_S):
    ...
    for entry in root.iterfind("./channel/item"):
        ...
        posted = _pubdate(entry.findtext("pubDate"))
        items.append(_item(
...
def search_bitsearch(query, timeout=HTTP_TIMEOUT_S):
    ...
    for doc in response.json().get("results") or ():
        items.append(_item(
            ...
            posted=_timestamp(doc.get("updatedAt")),
...
def search_knaben(query, timeout=HTTP_TIMEOUT_S):
    ...
    This is how 1337x results are covered: Knaben runs its own scraper against it
    ...
    response = _http().post(
        ...
        json={
            ...
            # not what a torrent search is for.
            "hide_unsafe": True,
            "hide_xxx": True,
        },
        timeout=timeout,
    ...
    for doc in response.json().get("hits") or ():
        ...
        items.append(_item(
            ...
            posted=_timestamp(doc.get("date")),
...
def _from_torznab(source, entry):
    """One Torznab RSS item as a normalized result row."""
    attrs = _attrs(entry)
    title = (entry.findtext("title") or "").strip()
    if not title:
        return None
...
def _from_prowlarr(source, doc):
    """One row of Prowlarr's own JSON search as a normalized result row."""
    if str(doc.get("protocol") or "torrent").lower() != "torrent":
        return None  # a Usenet release is not something a BitTorrent session can take
    ...
    return _item(
        ...
        posted=_timestamp(doc.get("publishDate")),
...
def search(query, timeout_s=SEARCH_TIMEOUT_S, on_site=None, stop=None,
    ...
    user's own feeds, looked up in prefs.

    Returns (items, answered, asked).
    """
    by_feed = {feed["name"]: feed for feed in feeds(prefs)}
    wanted = [source for source in (sources or all_sources(prefs))

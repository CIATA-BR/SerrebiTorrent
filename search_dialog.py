...
class TorrentSearchDialog(wx.Dialog):
    ...
    def __init__(self, parent, config_manager):
        super().__init__(parent, title="Search for Torrents",
                         size=(900, 600),
        ...
        query_label = wx.StaticText(panel, label="&Search for:")
        ...
        self.search_btn = wx.Button(panel, label="Sea&rch")
        ...
        self.sort_choice = wx.Choice(panel, choices=SORT_LABELS)
        self.sort_choice.SetName("Sort by")
        ...
        sources_btn = wx.Button(panel, label="Search si&tes...")
        sources_btn.Bind(wx.EVT_BUTTON, self.on_sources)
        feeds_btn = wx.Button(panel, label="My &indexers...")
        ...
        self.list.SetName("Results")
        self.list.SetHelpText(
            "Enter adds the selected torrents. Control C copies the magnet "
            "link.")
        ...
        self.status = wx.StaticText(panel, label="Type what to look for.")

        self.add_btn = wx.Button(panel, wx.ID_OK, "&Add selected")
    ...
    def on_search(self, event=None):
        ...
        if not query:
            self._say("Type what to look for first.")
            self.query.SetFocus()
            return
        prefs = self._prefs()
        ...
        if not sources:
            self._say("No indexers are switched on. Use Search sites.")
            return
        ...
        word = "indexer" if len(sources) == 1 else "indexers"
        self._say(f"Searching {len(sources)} {word}...")
    ...
    def _search_worker(self, query, sources, prefs, token, stop):
        ...
        try:
            ...
            wx.CallAfter(self._say, f"Search failed: {exc}")
    ...
    def _add_rows(self, token, source, items):
        ...
        if self._pending:
            self._say(f"{count} {noun} so far, {self._pending} still "
                      f"searching. Last: {source}.")
        else:
            self._say(f"{count} {noun}.")
    ...
    def _search_done(self, token):
        ...
        count = len(self.results)
        if not count:
            self._say("Nothing found. Try fewer words, or switch on more "
                      "indexers in Search sites.")
            self.query.SetFocus()
            return
        noun = "result" if count == 1 else "results"
        self._say(f"{count} {noun}.")
        ...
        self.list.SetName(f"Results, {count} {noun}")
    ...
    def on_sort(self, event=None):
        if not self.results:
            return
        self._repopulate()
        self._say(f"Sorted by {SORT_LABELS[self.sort_choice.GetSelection()]}.")
    ...
    def on_add(self, event=None):
        ...
        if not picked:
            self._say("Select a result first.")
            return
        self.chosen = picked
        self._stop.set()
    ...
    def _copy_magnet(self):
        ...
        if not links:
            self._say("Those results carry a tracker file rather than a "
                      "magnet link.")
            return
        if wx.TheClipboard.Open():
            wx.TheClipboard.SetData(wx.TextDataObject("\n".join(links)))
            wx.TheClipboard.Close()
            noun = "link" if len(links) == 1 else "links"
            self._say(f"Copied {len(links)} magnet {noun}.")
...
class SourcesDialog(wx.Dialog):
    ...
    def __init__(self, parent, config_manager):
        ...
        sizer = wx.BoxSizer(wx.VERTICAL)
        label = wx.StaticText(self, label="&Indexers to search:")
        self.checklist = wx.CheckListBox(self, choices=self.sources)
        self.checklist.SetName("Indexers to search")
    ...
    def summary(self):
        on = sum(1 for index in range(self.checklist.GetCount())
                 if self.checklist.IsChecked(index))
        noun = "indexer" if on == 1 else "indexers"
        return f"{on} {noun} switched on."
...
class IndexerDialog(wx.Dialog):
    ...
    def __init__(self, parent, feed=None):
        ...
        self.key_text.SetHelpText(
            "Copy it from Prowlarr or Jackett's own settings. Leave it empty "
...
class IndexersDialog(wx.Dialog):
    ...
    def on_remove(self, event=None):
        ...
        self._refresh(index)
        self.list.SetFocus()
        wx.MessageBox(f"Removed {removed['name']}.", "SerrebiTorrent",

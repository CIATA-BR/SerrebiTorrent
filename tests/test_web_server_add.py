...
def test_torrents_add_delegates_redirect_safety_to_single_downloader(app_client, fake_client, monkeypatch):
    monkeypatch.setattr(
        web_server,
        "_resolve_add_host",
    ...
    def reject_rebound_download(url):
        calls.append(url)
        raise ValueError("Torrent connection reached a private or local network address.")

    monkeypatch.setattr(web_server, "download_torrent_url", reject_rebound_download)
